...
from ultralytics.nn.modules import C2f, Classify, Detect
    ...
    def __call__(self, model=None) -> str:
        ...
        if fmt == "axelera":
            if model.task == "segment" and any(
                type(m).__name__ == "Segment26" for m in model.modules()
            ):
        ...
        if self.args.nms:
            ...
            if getattr(model, "end2end", False) or type(model.model[-1]).__name__ == "RTDETRDecoder":
        ...
        if fmt == "edgetpu":
            if not LINUX or ARM64:
                ...
                self.args.batch = 1
        if type(model).__name__ == "WorldModel":
            LOGGER.warning(
