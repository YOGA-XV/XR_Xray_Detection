...
from ultralytics.data.xr_preprocess import apply_xr_preprocess_tensor
    ...
    def preprocess(self, im: torch.Tensor | list[np.ndarray]) -> torch.Tensor:
        ...
        if not_tensor:
            ...
            if im.shape[-1] == 3:
                ...
                im = np.concatenate((im[..., 2::-1], im[..., 3:]), axis=-1)  # BGR+extra to RGB+extra
        ...
        if self.args.xr_pcn or self.args.xr_egi:
            im = apply_xr_preprocess_tensor(im, use_pcn=self.args.xr_pcn, use_egi=self.args.xr_egi)
        return im
