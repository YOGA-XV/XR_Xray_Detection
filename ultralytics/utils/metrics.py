...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    Methods:
        ...
        mean_results: Calculate mean of detected objects & return precision, recall, F1, mAP50, mAP75, and mAP50-95.
    ...
    def mean_results(self) -> list[float]:
        """Calculate mean of detected objects & return precision, recall, F1, mAP50, mAP75, and mAP50-95."""
        f1 = self.box.f1.mean() if len(self.box.f1) else 0.0
        return [self.box.mp, self.box.mr, f1, self.box.map50, self.box.map75, self.box.map]

    def class_result(self, i: int) -> tuple[float, float, float, float, float, float]:
        """Return the result of evaluating the performance of an object detection model on a specific class."""
        map75 = self.box.all_ap[i, 5] if len(self.box.all_ap) else 0.0
        return self.box.p[i], self.box.r[i], self.box.f1[i], self.box.ap50[i], map75, self.box.ap[i]
