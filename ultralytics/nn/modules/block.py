...
def _valid_group_count(channels: int, requested: int) -> int:
    """Return a positive group count that divides the channel count."""
    groups = max(1, min(int(requested), int(channels)))
...
class EMALite(nn.Module):
    """Lightweight edge-aware mixed attention block for XR detection features."""

    def __init__(self, c1: int, groups: int = 8, reduction: int = 4):
        """Initialize the EMA-Lite block.

        Args:
            c1 (int): Input and output channels.
            groups (int): GroupNorm groups for grouped feature normalization.
            reduction (int): Channel reduction ratio for attention generation.
        """
        super().__init__()
        hidden = max(c1 // max(int(reduction), 1), 8)
        self.dw = nn.Conv2d(c1, c1, 3, padding=1, groups=c1, bias=False)
        self.norm = nn.GroupNorm(_valid_group_count(c1, groups), c1)
        self.act = nn.SiLU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply residual spatial-channel attention while preserving feature shape."""
        b, c, h, w = x.shape
        y = self.act(self.norm(self.dw(x)))
        context_h = y.mean(dim=3, keepdim=True)
        context_w = y.mean(dim=2, keepdim=True).transpose(2, 3)
        context = self.spatial(torch.cat((context_h, context_w), dim=2))
        att_h, att_w = torch.split(context, (h, w), dim=2)
        spatial_att = torch.sigmoid(att_h * att_w.transpose(2, 3))
        channel_att = torch.sigmoid(self.channel(y))
        return x + x * spatial_att * channel_att


class AIFILite(nn.Module):
    """Single-block lightweight intra-scale feature interaction for P5 features."""

    def __init__(self, c1: int, reduction: int = 4, heads: int = 4, ffn_expansion: int = 2, dropout: float = 0.0):
        """Initialize the AIFI-Lite block.

        Args:
            c1 (int): Input and output channels.
            reduction (int): Channel reduction ratio before self-attention.
            heads (int): Attention head count.
            ffn_expansion (int): Expansion ratio of the local FFN branch.
            dropout (float): Dropout probability in the attention branch.
        """
        super().__init__()
        reduced = max(c1 // max(int(reduction), 1), 8)
        heads = max(1, min(int(heads), reduced))
        while reduced % heads:
            heads -= 1
        self.heads = max(heads, 1)
        self.head_dim = reduced // self.heads
        self.scale = self.head_dim**-0.5
        self.reduce = Conv(c1, reduced, 1)
        self.local = DWConv(reduced, reduced, 3)
        self.qkv = nn.Linear(reduced, reduced * 3)
        self.attn_drop = nn.Dropout(float(dropout))
        self.proj = nn.Linear(reduced, reduced)
        self.expand = Conv(reduced, c1, 1, act=False)
        hidden = max(c1 * max(int(ffn_expansion), 1), c1)
        self.ffn = nn.Sequential(Conv(c1, hidden, 1), DWConv(hidden, hidden, 3), Conv(hidden, c1, 1, act=False))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply reduced self-attention and a local FFN residual branch."""
        b, _, h, w = x.shape
        y = self.local(self.reduce(x))
        tokens = y.flatten(2).transpose(1, 2)
        qkv = self.qkv(tokens).reshape(b, h * w, 3, self.heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = self.attn_drop(attn.softmax(dim=-1))
        y = (attn @ v).transpose(1, 2).reshape(b, h * w, self.heads * self.head_dim)
        y = self.proj(y).transpose(1, 2).reshape(b, self.heads * self.head_dim, h, w)
        x = x + self.expand(y)
        return x + self.ffn(x)


class SPPFAIFILite(SPPF):
    """SPPF followed by AIFI-Lite while preserving original SPPF parameter names."""
    ...
    def __init__(
        ...
        """Initialize SPPF + AIFI-Lite for the P5 context branch."""
        super().__init__(c1, c2, k=k, n=3, shortcut=shortcut)
        self.aifi = AIFILite(c2, reduction=reduction, heads=heads, ffn_expansion=ffn_expansion, dropout=dropout)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply SPPF and then lightweight P5 self-attention."""
        return self.aifi(super().forward(x))
...
class C2fEMALite(C2f):
    """C2f followed by EMA-Lite while preserving original C2f parameter names."""
    ...
    def __init__(
        ...
        """Initialize C2f + EMA-Lite for P4/P5 feature enhancement."""
        super().__init__(c1, c2, n=n, shortcut=shortcut, g=g, e=e)
        self.ema = EMALite(c2, groups=ema_groups)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply C2f then residual spatial-channel attention."""
