...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        if first_conv not in updated_csd and first_conv in state_dict:
            adapted = adapt_input_conv_weight(state_dict[first_conv], csd[first_conv])
            if adapted.shape == state_dict[first_conv].shape:
                state_dict[first_conv].copy_(adapted)
                len_updated_csd += 1
        if verbose:
...
def adapt_input_conv_weight(target: torch.Tensor, source: torch.Tensor) -> torch.Tensor:
    """Adapt a pretrained input convolution tensor to a different input channel count.

    Existing channels are copied directly. If the target has extra channels, they are initialized from the mean of the
    source input-channel kernels, which is the Stage 2 policy for the Edge channel.
    """
    adapted = target.clone()
    if target.shape[-2:] != source.shape[-2:]:
        return adapted
    out_channels = min(target.shape[0], source.shape[0])
    in_channels = min(target.shape[1], source.shape[1])
    adapted[:out_channels, :in_channels] = source[:out_channels, :in_channels]
    if target.shape[1] > source.shape[1] and source.shape[1] >= 1:
        fill = source[:out_channels].mean(dim=1, keepdim=True)
        adapted[:out_channels, source.shape[1] : target.shape[1]] = fill.expand(
            -1, target.shape[1] - source.shape[1], -1, -1
        )
    return adapted
    ...
    try:
        with temporary_modules(
            ...
            attributes={
                ...
                "ultralytics.utils.loss.E2EDetectLoss": "ultralytics.utils.loss.E2EDetectLoss",
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f], *args]
        elif m in frozenset({EMALite, AIFILite}):
            args = [ch[f], *args]
            c2 = ch[f]
